...
public final class CreationOfficerForm {
    ...
    public static final String  CHAMP_OFFICEROF = "officerof";
    ...
    public pers_Officer inscrireUtilisateur( HttpServletRequest req ) {
        ...
        String officerof = getValeurChamp( req, CHAMP_OFFICEROF );

        pers_Officer officer = new pers_Officer();
        try {
            validationMail( email );
        } catch ( Exception e ) {
            setErreur( CHAMP_EMAIL, e.getMessage() );
        }
        officer.setMail( email );
        try {
        ...
        officer.setAdress( adress );

        Bank bank = new Bank();
        try {
            validationBank( officerof );
            bank.setName( officerof );
        } catch ( Exception e ) {
     ...
     */
    private void validationBank( String officerof ) throws Exception {
        if ( officerof != null && officerof.trim().length() != 0 && officerof.trim().length() < 3 ) {
